...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results, shape, isCalculating }) => {
  ...
  // Use a more descriptive label depending on the shape.
  const lengthLabel = shape === 'triangle' ? 'Side (a)' : 'Length (L)';
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-center">
            ...
            {results.length && (
                <div className="bg-indigo-50 p-4 rounded-lg">
                  {/* The dynamic label is used here */}
                  <p className="text-sm text-indigo-800">{lengthLabel}</p>
            ...
            )}{results.triangleLength && (
                <div className="bg-indigo-50 p-4 rounded-lg sm:col-span-2">
                  <p className="text-sm text-indigo-800">Side (a)</p>
                  <p className="text-2xl font-bold text-indigo-900">{results.triangleLength.toFixed(3)} <span className="text-lg">mm</span></p>
                </div>
            )}
          </div>
        </div>
        
        {/* This entire block will now only render if the shape is NOT a triangle AND NOT a circle. */}
